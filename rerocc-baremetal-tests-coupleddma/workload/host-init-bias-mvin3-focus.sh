...
BUILD_CONFIG="${ARTIFACT_DIR}/build-config-bias-mvin3-focus.txt"
OUT_BIN="${SCRIPT_DIR}/rerocc_lc_bias_mvin3_runtime_alias_focus.riscv"

echo "[rerocc-coupleddma-bias-mvin3-focus] target=${TARGET_PROFILE} num_cores=${NUM_CORES} num_gemmini=${NUM_GEMMINI} gemmini_base_id=${GEMMINI_BASE_ID} local_gemmini_id=${LOCAL_GEMMINI_ID}"
...
make -B -C "${BUILD_BAREMETAL_DIR}" \
  ...
  rerocc_lc_bias_mvin3_runtime_alias_focus-baremetal

cp -f "${BUILD_BAREMETAL_DIR}/rerocc_lc_bias_mvin3_runtime_alias_focus-baremetal" "${OUT_BIN}"
