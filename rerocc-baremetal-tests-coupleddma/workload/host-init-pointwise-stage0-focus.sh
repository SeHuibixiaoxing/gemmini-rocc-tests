...
BUILD_CONFIG="${ARTIFACT_DIR}/build-config-pointwise-stage0-focus.txt"
OUT_BIN="${SCRIPT_DIR}/rerocc_lc_pointwise_stage0_runtime_interleaved_focus.riscv"

echo "[rerocc-coupleddma-pointwise-stage0-focus] target=${TARGET_PROFILE} num_cores=${NUM_CORES} num_gemmini=${NUM_GEMMINI} gemmini_base_id=${GEMMINI_BASE_ID} local_gemmini_id=${LOCAL_GEMMINI_ID}"
...
make -B -C "${BUILD_BAREMETAL_DIR}" \
  ...
  rerocc_lc_pointwise_stage0_runtime_interleaved_focus-baremetal

cp -f "${BUILD_BAREMETAL_DIR}/rerocc_lc_pointwise_stage0_runtime_interleaved_focus-baremetal" "${OUT_BIN}"
