...
BUILD_CONFIG="${ARTIFACT_DIR}/build-config-export-dma-bertmini-repro.txt"
OUT_BIN="${SCRIPT_DIR}/rerocc_lc_export_dma_bertmini_repro.riscv"

echo "[rerocc-coupleddma-export-dma-bertmini-repro] target=${TARGET_PROFILE} num_cores=${NUM_CORES} num_gemmini=${NUM_GEMMINI} num_dma=${NUM_DMA} gemmini_base_id=${GEMMINI_BASE_ID} dma_base_id=${DMA_BASE_ID} stage0_bytes=${STAGE0_BYTES} stage1_bytes=${STAGE1_BYTES} repeat_count=${REPEAT_COUNT} wait_spins=${WAIT_SPINS}"
...
make -B -C "${BUILD_BAREMETAL_DIR}" \
  ...
  rerocc_lc_export_dma_bertmini_repro-baremetal

cp -f "${BUILD_BAREMETAL_DIR}/rerocc_lc_export_dma_bertmini_repro-baremetal" "${OUT_BIN}"
