...
BUILD_CONFIG="${ARTIFACT_DIR}/build-config-explicit-interleaved.txt"
OUT_BIN="${SCRIPT_DIR}/rerocc_lc_resadd_explicit_interleaved.riscv"

echo "[rerocc-coupleddma-explicit-interleaved] target=${TARGET_PROFILE} num_cores=${NUM_CORES} num_gemmini=${NUM_GEMMINI} gemmini_base_id=${GEMMINI_BASE_ID} local_gemmini_id=${LOCAL_GEMMINI_ID}"
...
EXTRA_DEFS="${EXTRA_DEFS} -DREROCC_STALL_DIAG_ONLY=${REROCC_STALL_DIAG_ONLY:-0}"
EXTRA_DEFS="${EXTRA_DEFS} -DREROCC_RUNTIME_STYLE_ONLY=${REROCC_RUNTIME_STYLE_ONLY:-1}"
...
cp -f "${BUILD_BAREMETAL_DIR}/rerocc_lc_resadd_explicit_interleaved-baremetal" "${OUT_BIN}"
