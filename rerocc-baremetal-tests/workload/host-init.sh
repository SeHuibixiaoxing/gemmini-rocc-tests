...
OUT_BIN="${SCRIPT_DIR}/rerocc_lc_matrix_baremetal.riscv"

echo "[rerocc-baremetal] target=${TARGET_PROFILE} matrix=${MATRIX_MODE} num_cores=${NUM_CORES} num_gemmini=${NUM_GEMMINI} num_dma=${NUM_DMA} bytes=${BYTES} gemmini_base_id=${GEMMINI_BASE_ID} dma_base_id=${DMA_BASE_ID}"
...
make -B -C "${BUILD_BAREMETAL_DIR}" \
  ...
  rerocc_lc_matrix_baremetal-baremetal

cp -f "${BUILD_BAREMETAL_DIR}/rerocc_lc_matrix_baremetal-baremetal" "${OUT_BIN}"
