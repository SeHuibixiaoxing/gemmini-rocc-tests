...
UARTPROBE_CAPTURE_ECHO_STDOUT="${UARTPROBE_CAPTURE_ECHO_STDOUT:-0}"
...
mkdir -p "${LOG_DIR}"
rm -f "${SPARSE_LOG_PATH}" "${DEEP_LOG_PATH}" "${STATUS_PATH}" "${RUNNER_STAGE_PATH}" "${BINARY_STAGE_PATH}" "${PROC_STAGE_PATH}" "${LOG_PIPE_PATH}"
...
echo "[firemarshal-uartprobe] wrapper-enter proc_diag=${UARTPROBE_PROC_DIAG_ENABLE} polls=${UARTPROBE_PROC_DIAG_POLLS} sleep=${UARTPROBE_PROC_DIAG_SLEEP_SECONDS} kill=${UARTPROBE_PROC_DIAG_KILL_ON_STUCK} echo_stdout=${UARTPROBE_CAPTURE_ECHO_STDOUT}"

write_status "starting" ""
flush_capture_state

if [ "${UARTPROBE_CAPTURE_ECHO_STDOUT}" = "0" ]; then
