...
script_name="${PAIRDUMMY_TRACERV_SCRIPT_NAME_OVERRIDE:-$(basename "${BASH_SOURCE[0]}")}"
session_prefix="${PAIRDUMMY_TRACERV_SESSION_PREFIX:-pairdummy-sbus128-tracerv}"
workflow_description="${PAIRDUMMY_TRACERV_DESCRIPTION:-Isolated TracerV debug workflow for the 12-pair sbus128 dummy-model pipeline-runtime path.}"
trigger_usage_desc="${PAIRDUMMY_TRACERV_TRIGGER_USAGE_DESC:-enables PC-triggered TracerV capture}"
show_note="${PAIRDUMMY_TRACERV_SHOW_NOTE:-do-not-infer-entrypoint-from-workload-overlay-root-firemarshal.sh}"
show_selector="${PAIRDUMMY_TRACERV_SHOW_SELECTOR:-2}"
show_start_pc="${PAIRDUMMY_TRACERV_SHOW_START_PC:-2d9ba}"
show_end_pc="${PAIRDUMMY_TRACERV_SHOW_END_PC:-ffffffffffffffff}"

workload_json="${PAIRDUMMY_TRACERV_WORKLOAD_JSON:-generators/gemmini/software/gemmini-rocc-tests/rerocc-linux-tests-coupleddma/workload/rerocc-lc-linux-coupleddma-bertmini-pipeline-runtime-batch8-fileonly-sync-pairdummy-tracerv.json}"
runtime_cfg="${PAIRDUMMY_TRACERV_RUNTIME_CFG:-/home/ubuntu/chipyard/sims/firesim/deploy/config_runtime_f2_gemmini_rerocc_pairmanager_dummy16x16_4c12p12_sbus128_linux_bertmini_pipeline_runtime_batch8_fileonly_sync_tracerv.yaml}"
hwdb_cfg="${PAIRDUMMY_TRACERV_HWDB_CFG:-/home/ubuntu/chipyard/sims/firesim/deploy/config_hwdb_f2_gemmini_rerocc_pairmanager_dummy16x16_4c12p12_sbus128_bertmini.yaml}"
build_recipes_cfg="${PAIRDUMMY_TRACERV_BUILD_RECIPES_CFG:-/home/ubuntu/chipyard/sims/firesim-staging/sample_config_build_recipes.yaml}"
...
usage() {
  cat <<EOF
Usage: ${script_name} <command> [private-ip]
...
run_firemarshal() {
  local subcmd="$1"
  local session="${session_prefix}-${subcmd}-$(timestamp)"
...
tracing_selector=${show_selector}
tracing_start_pc=${show_start_pc}
tracing_end_pc=${show_end_pc}
...
debug_preflight() {
  ...
  echo "debug_preflight_recommended_order=artifact-audit -> debug-preflight -> isolated-tracerv-rerun -> trace-analysis"
