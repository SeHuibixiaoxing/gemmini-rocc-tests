...
usage() {
  cat <<'EOF'
Usage: run_pairdummy_cfg32_gdbserver_no_dma_compute_thin.sh <run-host-private-ip> <guest-ip-or-endpoint> [local-port]
...
This is the low-overhead follow-up to the dense compute frontier. It skips the
first seven manager hits, tracks manager 7 through pointwise issue/fence/drain/
release, and stops if prt_gemm_conv_run returns.
EOF
}
...
gdb_file="${script_dir}/gdb/sbus64_no_dma_segment1_stage0_compute_thin.gdb"
