...
usage() {
  cat <<'EOF'
Usage: run_pairdummy_cfg32_gdbserver_export_page_frontier.sh <run-host-private-ip> <guest-ip-or-endpoint> [local-port]

Attach to the cfg32 NIC gdbserver workload after the guest-side GDB marker has
been configured to stop at one export DMA page. The helper breaks only on
prt_gdb_marker_stop(), then arms one-shot breakpoints for the immediate
dma_submit_wait_annotated_scoped() path. This avoids a hot host-side
conditional breakpoint on every DMA wait.
...
cmd_dir="${cy_dir}/tmp/firesim-aws-f2/gdbserver-tests/export-page-frontier-${stamp}"
post_cmds="${cmd_dir}/export-page-frontier.gdb"
...
echo "[pairdummy-export-page-frontier] post_cmds=${post_cmds}"
echo "[pairdummy-export-page-frontier] expected stage=${stage} manager=${manager} tensor=${tensor} page=${page}"
