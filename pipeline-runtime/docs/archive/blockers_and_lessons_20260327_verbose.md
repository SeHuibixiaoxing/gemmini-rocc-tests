...
- 深日志不要再全程打开
- 现在 runtime 已支持“按窗口激活深日志”：
  - `--deep-log-enable 1`
  - `--deep-log-segment <n>`
  - `--deep-log-global-stage <n>`
  - `--deep-log-local-stage <n>`
  - `--deep-log-subbatch <n>`
  - `--deep-log-stage-radius <n>`
  - `--deep-log-subbatch-radius <n>`
- 这套 gate 只影响深层热点日志：
  - Gemmini raw/crit/phase
  - runtime hot/raw
- 粗粒度 marker/progress 仍保留，用来把 run 推到疑似卡点附近
...
- `--deep-log-subbatch 0`
- 如需把卡点前后一起抓进来，再加：
  - `--deep-log-stage-radius 1`
  - `--deep-log-subbatch-radius 0`

这样可以把大量 pointwise/Gemmini 细日志压缩到当前最可疑窗口，避免再次因为全程 UART 噪声把执行速度拖慢。
...
## 15. 2026-03-27 新增最深卡点：`compute-accum` 前一条 critical log 本身

- 这轮新的最深 Linux/F2 边界已经不在旧的 bias `mvin3` 周边。
- 最新 live run 在：
  - `segment=2 sink-progress=14/16`
  - `global_stage=3`
  - `split=1`
  - `worker stage=0 subbatch=0 gemm-enter`
  之后停住。

- 远端 `uartlog` 与 `heartbeat.csv` 的联合证据：
  - `2026-03-27 14:54:32 UTC` 到 `2026-03-27 15:04:33 UTC`
  - `uartlog` 大小保持不变
  - `heartbeat.csv` 持续推进
- 这说明：
  - 不是 Linux boot 慢
...
- 最后一段 UART 原始尾部停在：
  - `matmul-os-compute-preloaded-debug-return`
  - `matmul-os-post-compute-preloaded0`
  - `matmul-os compute-iter tag=first-accum ...`
  - `matmul-os-pre-compute-accum1`
  - `matmul-os compute-accum1 ...`
  - `matmul-os-compute-accum-debug-enter`
  - `matmul-os-compute-accum-debug-a ...`
  - `matmul-os-compute-accum-debug-bd ...`
  - `matmul-os-compute-accum-debug-shape ...`
  - 然后半截停在：`[gcr`

- 这和源码精确对上后，`[gcr` 后面本来应该是：
  - `matmul-os-compute-accum-debug-rs rs1=... rs2=...`
- 因此当前最可疑结论不是“先卡在 `k_COMPUTE_ACCUMULATE` 指令本体”，而是：
  - guest 卡在这条 deep critical log 的 `write()` 路径里
  - 也就是 Linux/FireSim UART 背压再次把 guest 阻塞在日志写出上

- 这个判断成立的原因：
  - 最后一行停在 tag 前缀中间，而不是完整 marker 之后
  - 下一条源码语句正好是长 `PRT_GEMMINI_CRIT_LOG(...)`
  - 当时 Linux 路径的 critical/raw log helper 还是“阻塞式 write-until-drained”

## 16. 2026-03-27 新增修补：Linux 日志改成 nonblocking best-effort

- 已在以下位置把 Linux 路径的 direct-write helper 改成：
  - 通过 `/proc/self/fd/2` 打开独立的 `O_NONBLOCK` 日志 fd
  - `write()` 遇到 `EINTR` 重试
  - 其他背压/不可写情况直接放弃剩余字节，不再阻塞 guest
- 修改文件：
  - `include/gemmini.h`
  - `pipeline-runtime/include/prt_progress.h`

- 额外新增了超短 boundary marker：
  - `compute_preloaded` 前后：`[graw] cp0` / `[graw] cp1`
  - `compute_accumulate` 前后：`[graw] ca0` / `[graw] ca1`
- 目的：
  - 下次如果仍卡住，可以直接区分：
    - 卡在 log 写出之前
    - 卡在 compute 指令本体
...
## 17. 2026-03-27 新增修补：runworkload watchdog 默认延长到 3 小时

- `scripts/firesim-tmux-run.sh` 的 `runworkload` watchdog 默认值已从：
  - `5400s`
  改到：
  - `10800s`
- 这样下次长时间 Linux/F2 调试时，不会太早被 watchdog 自动回收。
...
## 41. 2026-03-27 当前最深 live dispatch 路径已经收敛

- 这轮在 live `uartlog` 上重新抽取 stage0 的调度路径后，已经确认：
  - `worker stage=0 subbatch=0 task-ready op=1 split=2 acc=0 dma=2 tiles=2 mgr0=0 mgr1=1`
  - `gemm-issue-conv stage=0 dispatch=oc-split`
  - 第一块 tile 先走：
    - `conv-sync-strided stage=0 mgr=0`
  - 随后进入：
    - canonical pointwise matmul fallback
    - fallback type = `OS`
    - 第一发 bias 路径停在：
      - `[graw] bm0ca`

- 这说明当前卡点不是：
  - single-manager 单 stage 简化路径
  - 也不是 pack/repack 的 split-OC 路径

- 当前真实最深软件路径应记录为：
  - `run_conv_oc_split()`
  - `conv_call_for_manager_sync_strided()`
  - `prt_run_pointwise_matmul_fallback_strided_scoped()`
  - `tiled_matmul_nn_stride_auto()`
  - `tiled_matmul_auto()`
  - `sp_tiled_matmul_os()`
  - first `bias mvin3`

## 42. 2026-03-27 新增 hart / ReRoCC CSR 快照断点

- 为了判断这次 `bm0ca` stall 前是否发生了 hart 迁移，或 `opc3/cfg`
  绑定被别的控制路径改坏，这轮又新增了两类快照：

- runtime 侧：
  - `conv-sync-strided acquire-snapshot`
  - `conv-sync-strided pointwise-precall-snapshot`
  - 以及对应的 `conv-sync` / `conv-nb` 快照

- Gemmini 侧：
  - `matmul-os-biascfg-post-ld-snapshot`
  - `matmul-os-bias-mvin3-debug-pre-issue`
  - `matmul-os-bias-mvin3-debug-post-return`
...
- 这批快照的目的：
  - 证明或排除“stage thread 在 acquire 之后迁移到另一个 CPU/hart”
  - 证明或排除“`opc3` 在 first `bias mvin3` 前被别的路径重绑”
  - 证明或排除“stage0 exec cfg 和 xlate control cfg 状态不一致”

## 43. 2026-03-27 本轮静态复查：当前最可疑方向仍是 runtime-only control/binding state

- 目前又静态复查了一遍 `bm0ca` 前的软件路径：
  - `run_conv_oc_split()` 的 direct-strided pointwise split
  - `sub.weights = weights + oc_beg`
  - `sub.bias = bias + oc_beg`
  - `sub.output = output + oc_beg`
  - 同时保持原始 `weight_stride/out_stride`

- 对 canonical pointwise 的 NHWC / `[k_rows][out_channels]` 权重布局来说，
  上述 slicing 在静态上是自洽的，暂时没有发现明显的：
  - 指针切片错误
  - stride 错误
  - `repeating_bias` 语义错误

- 因而当前优先级不应回退到“先猜布局/数值错误”，而应继续优先验证：
  - hart 迁移
  - `opc3` 绑定污染
  - cfg state 被 runtime control path 破坏

## 44. 2026-03-27 新结论：snapshot helper 执行到了，但单大行 snapshot 日志本身失效

- 对最新一次 Linux/F2 live run 抓回的 `uartlog` 做了更细的字节级复查后，可以确认：
  - `bm0ce` 已经打印
  - `bm0ca` 也已经打印
  - 说明 `prt_gemmini_rr_snapshot_impl("matmul-os-bias-mvin3-debug-pre-issue")`
    之前的代码已经执行过，并且 helper 调用之后也继续往下走了
  - 但 `bm0ce` 与 `bm0ca` 之间没有任何隐藏的 snapshot 字节流

- 进一步对正式 Linux 二进制做反汇编确认后，`sp_tiled_matmul_os()` 中
  `bias mvin3` 前的顺序已经确定是：
  - phase log
  - `prt_gemmini_rr_snapshot_impl("...pre-issue")`
  - `[graw] bm0ca`
  - first `mvin3`

- 这说明：
  - 缺失的 snapshot 行不是“编译没进 binary”
  - 也不是“根本没走到 helper”
  - 而是原来那种单条超长 snapshot log 在 Linux/F2 上存在
    “helper 返回了，但整行没有落到 uartlog” 的现象

- 当前采取的修补：
  - `gemmini.h` 中的 `prt_gemmini_rr_snapshot_impl()`
    已从单条大日志改成三条短日志：
    - `... base`
    - `... opc`
    - `... cfg`
  - `prt_gemmini_adapter.c` 中的 `prt_log_rr_binding_snapshot()`
    也同样拆成三条短日志：
    - `... base`
    - `... opc`
    - `... cfg`

- 这样做的目的不是“修功能”，而是提高下一轮 Linux/F2 的观测可靠性：
  - 在不改变 `bm0ca` 附近功能路径的前提下，
    先稳定拿到：
    - 当前 CPU / hart 观测值
    - `opc0..opc3`
    - `cfg_scope`
...
- 当前正式产物状态：
  - `build/rerocc-linux-tests/rerocc_pipeline_runtime-linux`
    已按新代码重建
  - `rerocc-linux-tests-coupleddma/workload/overlay/root/rerocc-linux-tests/pipeline-runtime/rerocc_pipeline_runtime-linux`
    已与正式 build 产物重新同步
  - 下一轮 FireSim 需要重新走：
    - `launchrunfarm`
    - `infrasetup`
